...
Compare this with the embedding $$(\mathbb{A}^1_K)^{an}\hookrightarrow\prod_{f\in K[T]}[0,\infty)\cong\prod_{f\in K[T]}(-\infty,\infty]$$ seen in the [previous post](/posts/2022/05/berkovich-line-1/). The product was held over all polynomials back then, and now we reduced it to linear monic polynomials only!
