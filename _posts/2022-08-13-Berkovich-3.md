...
<!-- When dimensions are low (say $n\leq 3$) and $X=V(f)$ is smooth of codimension 1, then sketching $V(\mathrm{trop}(f))$ is more or less a visual exercise.
 ...
 >
 > This divides the space into 4 cells, where each has $x_1,x_2,x_3$ or $(-1)$ valuated to a least value.
...
Compare this with the embedding $$(\mathbb{A}^1_K)^{an}\hookrightarrow\prod_{f\in K[T]}[0,\infty)\cong\prod_{f\in K[T]}(-\infty,\infty]$$ seen in the [previous post](/posts/2022/05/berkovich-line-1/#evaluation-embedding). The product was held over all polynomials back then, and now we reduced it to linear monic polynomials only!
