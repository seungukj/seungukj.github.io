...
Adding $v_p(p)=1$ and $v_p(a)=0$ if $(a,p)=1$, the axioms completely determines $v_p$. Morally, if $a\in\mathbb{Z}$, the valuation $v_p(a)$ find the number $v$ in which $p^v \vert\vert a$ (i.e., $p^v\mid a$ but $p^{v+1}\mid a$) holds.
