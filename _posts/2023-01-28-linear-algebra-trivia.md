---
title: 'Miscellaneuous Linear Algebra Things'
date: 2023-01-28
...
Some list of trivias found while teaching Linear Algebra in Fall 2022 quarter.
...
These sets are orthogonal, and one can normalize them to make an orthonormal basis (of $\mathbb{R}^3$). However, often the normalization involves many square roots that does not give a rational vector.

 > Of course there are some integer vectors that do give rational vectors after normalization, like
 >
 ...
 >
 > But as this corresponds to solving diophantine equations of the sort $x^2+y^2+z^2(+w^2)=n^2$, there are only few examples with small numbers.

But if one looks through these square roots, one observes a bizarre pattern. For two examples above, normalizations give
...
Observe that $\sqrt{3}\cdot\sqrt{2}\cdot\sqrt{6}=6$ and $\sqrt{6}\cdot\sqrt{5}\cdot\sqrt{30}=30$; the products of norms are integers! This tells that norms of orthogonal set of integral vectors are not just random square roots, but rather having a pattern on their own.
...
Of course one already has the result, by observing that \eqref{eqn:1} gives $\det(P^\dagger P)=(\det P)^2=\prod_i\vert\mathbf{v}_i\vert^2$, which gives \eqref{eqn:2}. The proof stated above is designed to show that how SVD is crucial in the proof. Having said so, I wonder the <a href="#prop1">Proposition</a> above has a purely number-theoretical proof.
...
we have a much less effort to find the trace and determinant of the matrix and attempt to solve

$$\lambda_1+\lambda_2=a+d,\quad \lambda_1\lambda_2=ad-bc.\label{eqn:3}$$

Evidently, trace and determinant are not everything, even for $3\times 3$ matrices. Theoretically, knowing traces of powers of the matrix is good enough. This follows from an easy fact
...
where the last line is a complicated way of saying $\lambda_1\lambda_2\lambda_3=\det A$. (See [Wikipedia](https://en.wikipedia.org/wiki/Characteristic_polynomial#Properties) for more general formula.)

It feels odd to see that the degree 2 [elementary symmetric polynomial](https://en.wikipedia.org/wiki/Elementary_symmetric_polynomial) $\lambda_1\lambda_2+\lambda_2\lambda_3+\lambda_3\lambda_1$ is having a somewhat complicated formula (modulo trace and determinant). It turns out that this formula can be simplified, with a cost of having a new 'determinant-like formula.' as sketched below.
...
I will temporarily call this as the *2-block determinant-trace*.

 > **Example.** Consider the $3\times 3$ matrix
 >
 > $$\begin{bmatrix} 1 & 0 & -1 \\ 0 & 1 & 2 \\ -1 & 2 & 5 \end{bmatrix}.$$
 >
 > Then the trace of this is 7, determinant is 0 ((row 3)=2.(row 2)-(row 1)), and 2-block determinant-trace is $1\cdot 1 + 1\cdot 5 + 1\cdot 5 - (0\cdot 0 + (-1)\cdot(-1) + 2\cdot 2) = 11-5=6$. This leads us to solve
 >
...
It is not hard to generalilze the computation above, and show that
